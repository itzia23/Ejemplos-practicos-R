...
En este código, el paquete readxl se carga utilizando library().
Luego, el archivo de Excel se lee en R utilizando la función read_excel().
La función de indexación se utiliza para seleccionar todas las columnas excepto la que se desea eliminar. La columna a eliminar se especifica utilizando la función %in%.
```{r, message=FALSE}
